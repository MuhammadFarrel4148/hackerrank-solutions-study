...
function taumBday(b, w, bc, wc, z) {
    ...
    let wcBig = BigInt(wc);
    let zBig = BigInt(z);
    let priceBlack = bcBig;
    let priceWhite = wcBig;

    if(wcBig + zBig < priceBlack) {
        priceBlack = wcBig + zBig;
    };

    if(bcBig + zBig < priceWhite) {
        priceWhite = bcBig + zBig;
    };

    const result = (bBig * priceBlack) + (wBig * priceWhite);
